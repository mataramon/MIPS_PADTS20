    ...
    switch (instr.opcode) {
        ...
        case OP_SPECIAL:
            switch (instr.funct_1) {
                case S1F1_ADD: instr.name = MIPS32::ADD; break; 
                case S1F1_ADDU: instr.name = MIPS32::ADDU; break;  
                case S1F1_SUBU: instr.name = MIPS32::SUBU; break;
                case S1F1_DIV: instr.name = MIPS32::DIV; break;  // NOT TESTED bad implementation
        	case S1F1_JALR:	instr.name = MIPS32::JALR; break;
                case S1F1_JR:  instr.name = MIPS32::JR; break;
                case S1F1_TEQ: instr.name = MIPS32::TEQ; break;
                case S1F1_MOVN: instr.name = MIPS32::MOVN; break;
                case S1F1_SLL: instr.name = MIPS32::SLL; break;
                case S1F1_SRL: instr.name = MIPS32::SRL; break;

/*	        NONE OF THESE ARE IMPLEMENTED!!!		
                case S1F1_TNE: instr.name = MIPS32::TNE; break;
                case S1F1_SLT: instr.name = MIPS32::SLT; break;

                case S1F1_MFLO: instr.name = MIPS32::MFLO; break;
                case S1F1_SLLV: instr.name = MIPS32::SLLV; break;
                case S1F1_MFHI: instr.name = MIPS32::MFHI; break;
                case S1F1_SUB:  instr.name = MIPS32::SUB; break;
                case S1F1_TLT:  instr.name = MIPS32::TLT; break;
                case S1F1_SRA:  instr.name = MIPS32::SRA; break;
                case S1F1_TGEU: instr.name = MIPS32::TGEU; break;*/

                case S1F1_SYSCALL: instr.name = MIPS32::SYSCALL; break;



                default: instr.name = MIPS32::IERR; 
			printf("Instruction Not FOUND: 0x%08x\n",idata); break;
            } break;
...
/*        case OP_REGIMM:
	...
	case OP_ADDI: 	instr.name = MIPS32::ADDI; break;  
        case OP_SLTIU: 	instr.name = MIPS32::SLTIU; break;
        case OP_SLTI: 	instr.name = MIPS32::SLTI; break;
        case OP_J: 	instr.name = MIPS32::J; break;
        case OP_JAL:	instr.name = MIPS32::JAL; break;
        case OP_BNE:	instr.name = MIPS32::BNE; break;
        case OP_BNEL: 	instr.name = MIPS32::BNEL; break;
        case OP_BLEZL: 	instr.name = MIPS32::BLEZL; break;
        case OP_BLEZ: 	instr.name = MIPS32::BLEZ; break;
        case OP_SB: 	instr.name = MIPS32::SB; break;
	case OP_LBU : 	instr.name = MIPS32::LBU; break;
	case OP_LB :    instr.name = MIPS32::LB; break;
	case OP_SH :    instr.name = MIPS32::SH; break;
	case OP_LHU :   instr.name = MIPS32::LHU; break;
	case OP_LH: 	instr.name = MIPS32::LH; break;
        case OP_LL: 	instr.name = MIPS32::LL; break;
	case OP_SW: 	instr.name = MIPS32::SW; break;
        case OP_LW: 	instr.name = MIPS32::LW; break;
        case OP_ANDI: 	instr.name = MIPS32::ANDI; break;
	case OP_ORI :   instr.name = MIPS32::ORI; break;
...
void mips32::execute(MIPS32::instr_t instr)
{
    ...
    switch (instr.name) {
        ...
        case MIPS32::ADDI:
            if (verbose) printf("Catched ADDI  \t-> 0x%08x: ADD r%d, r%d, 0x%08x \n", PC, instr.rs, instr.rt, instr.immediate);
            temp64 = GPR[instr.rs] + instr.immediate;
            if ((temp64 & 0x100000000) && (!(temp64 & 0x080000000)))
                SignalException(MIPS32::integerOverflow);
            else
                GPR[instr.rt] = temp64;
        ...
        case MIPS32::ADDIU: // Correct :-)
            if (verbose) printf("Catched ADDIU \t-> 0x%08x: ADDIU r%d, r%d, 0x%08x \n", PC, instr.rs, instr.rt, instr.immediate);
        ...
        case MIPS32::SLTIU: // Set on Less Than Immediate Unsigned // Correct :-)
            ...
            GPR[instr.rt] = ( utemp16_rs < utemp16_imm ) ? 1 : 0; 
            next_PC = PC + 4;
            cntr.arithmetic++;
            break;
        ...
        case MIPS32::SLTI: // Correct :-)
            ...
            utemp16_rs= GPR[instr.rs];
            GPR[instr.rt] = ( utemp16_rs < instr.immediate ) ? 1 : 0;
            next_PC = PC + 4;
            cntr.arithmetic++;
            break;

        case MIPS32::DIV: // Divide Word
            if (verbose) printf("CHECK HI/LO? Catched DIV \t-> 0x%08x: DIV r%d, r%d\n", PC, instr.rs, instr.rt);
            int32_t q, r;
        ...
        case MIPS32::BLEZ: // TODO: Debug
            if( verbose ) printf("Catched BLEZ \t-> 0x%08x: BLEZ r%d, r%d, 0x%08x\n", PC, instr.rs, instr.rt, instr.offset);
	    target_offset = instr.offset << 2;            
            if(GPR[instr.rs] <= 0 ) {
                next_PC=PC+target_offset;
            }
...
/*        case MIPS32::NOP: // No operation // Correct :-)
	...
	case MIPS32::LHU: // Load Byte Unsigned// TODO
	    ...
	    temp32 = memword >> (8 * byte);	    
            ...
            if (verbose) printf("\tContents Raw:%d, Dec:%d, Hex:0x%08x\n",memword, memword >> (8 * byte), memword);
            next_PC = PC + 4;
	    cout << "fin de lhu" << endl;
            cntr.memory++;
	    break;

/*	    case MIPS32::LWL: // TODO: debug this
            if (verbose) printf("-wrong Catched LWL \t-> 0x%08x: LH r%d, 0x%08x\n", PC, instr.rt, instr.offset);
        ...
        case MIPS32::SW: // Correct :-)
            if (verbose) printf("Catched SW \t-> 0x%08x: SW b%d, r%d, 0x%08x\n", PC, instr.base, instr.rt, instr.offset);
            vAddr=instr.offset + GPR[instr.base];
        ...
        case MIPS32::LW: // Correct :-) 
            if (verbose) printf("Catched LW\t-> 0x%08x: LW b%d, r%d, 0x%08x\n", PC, instr.base, instr.rt, instr.offset);
            vAddr=instr.offset + GPR[instr.base];
            if( (vAddr & 0x00000003) != 0 )
                SignalException(MIPS32::AddressError);
            pAddr=AddressTranslation(vAddr, MIPS32::DATA, MIPS32::STORE);
        ...
        case MIPS32::LL: //Debug this
            if (verbose) printf("Catched LL\t-> 0x%08x: LL b%d, r%d, 0x%08x\n", PC, instr.base, instr.rt, instr.offset);
        ...
        case MIPS32::MOVN: // Move Conditional on Not Zero
            ...
            // Shift Instructions
            // ************************
	case MIPS32::SRL: //Shift Word Right Logical
	    if (verbose) printf("Catched SRL \t-> 0x%08x: SRL r%d, r%d, r%d\n", PC, instr.rt, instr.rd, instr.sa);
	    temp32 = GPR[instr.rt] >> instr.sa;
	    GPR[instr.rd] = temp32;
	    next_PC = PC + 4;
            cntr.shift++;
	    break;

	case MIPS32::SLL: //Shift Word Right Logical
	    if (verbose) printf("Catched SLL \t-> 0x%08x: SLL r%d, r%d, r%d\n", PC, instr.rt, instr.rd, instr.sa);
	    temp32 = GPR[instr.rt] << instr.sa;
	    GPR[instr.rd] = temp32;
	    next_PC = PC + 4;
            cntr.shift++;
        ...
        case MIPS32::IERR:
			printf("OPCODE MISSING: 0x%08x",instr.opcode); 
